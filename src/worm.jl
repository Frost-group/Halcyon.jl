...
"""
    energy_components(cfg::WormConfiguration, sys::System) -> (K, V, E)
    
Return Kinetic (K), Potential (V), and Total (E) energy components.
Based on the thermodynamic estimator.
"""
function energy_components(cfg::WormConfiguration, sys::System)
    ...
    # Components
    K = (D * N / (2 * δτ)) - (spring_sum / (4 * λ * δτ^2 * M))
    V = (int_contribution / M)
    E = K + V
    return K, V, E
end
