...
Base.@kwdef struct System{D,TV<:ExternalPotential,TU<:PairPotential}
    M::Int
    N::Int
    m::Float64 = 1.0
    β::Float64 = 100.0
...
function System(M::Integer, N::Integer; m::Float64=1.0, D::Integer=3, β::Float64=100.0,
    ...
    return System{Int(D),TV,TU}(M=Int(M), N=Int(N), m=m, β=β, V=V, U=U, λ=λ, τ=τval, L=L, statistics=statistics)
end

Base.show(io::IO, s::System{D}) where {D} = print(io,
    "PIMC system with $(s.M) beads and $(s.N) particles. \n" *
    "β=$(s.β), τ=$(s.τ). V=$(s.V) U=$(s.U)\n" *
    "Particle m of $(s.m), λ=$(s.λ). $(D) spatial dimensions." *
    "Box length $(s.L).")

# Allow seamless `sys.D` property access
Base.getproperty(sys::System{D}, sym::Symbol) where {D} = sym === :D ? D : getfield(sys, sym)
...
"""
@kwdef mutable struct WormConfiguration{D}
    sector::Sector = Z_SECTOR
    next::Vector{Int}
    prev::Vector{Int}
    i_head::Int = 1
    i_tail::Int = 1
    r_c_head::SVector{D,Float64}
    r::Array{Float64,3}
    w::Matrix{Int}
end
