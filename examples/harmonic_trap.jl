...
qho_ground_state_energy(ω::AbstractFloat) = 0.5 * ℏ * ω
qho_ground_state_density(x::AbstractFloat, ω::AbstractFloat) = sqrt(ω / (π * ℏ)) * exp(-ω * x^2 / ℏ)
qho_finite_temp_energy(ω::AbstractFloat, β::AbstractFloat) = (ℏ * ω / 2) * coth(β * ℏ * ω / 2)
qho_finite_temp_density(x::AbstractFloat, ω::AbstractFloat, β::AbstractFloat) = sqrt(m * ω / (2π * ℏ * sinh(β * ℏ * ω))) * exp(-(m * ω * x^2 / (2ℏ)) * tanh(β * ℏ * ω / 2))
...
println("qho ground state energy: $(qho_ground_state_energy(ω))")
println("qho finite temp energy (β=$β): $(qho_finite_temp_energy(ω, β))")
